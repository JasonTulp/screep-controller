...
use screeps::{
    ...
    find,
    local::ObjectId,
    objects::{ConstructionSite, Creep, Room, Source},
    prelude::*,
};
...
impl StateController {
    ...
    fn find_nearest_source(&mut self, creep: &Creep, room: &Room) -> Option<ObjectId<Source>> {
        let sources = room.find(find::SOURCES_ACTIVE, None);
        if sources.is_empty() {
            return None;
        }
        // Find the nearest source
        let nearest_source = sources
            .iter()
            .min_by_key(|source| creep.pos().get_range_to(source.pos()))?;
