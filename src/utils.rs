...
use screeps::{find, ConstructionSite, Creep, FindConstant, HasId, HasPosition, ObjectId, ResourceType, Room, StructureObject};

pub mod prelude {
    pub use {
        super::find_nearest_construction_site,
        super::find_nearest_object,
        super::get_total_upgrade_energy,
    };
}

// Gets the nearest object based on distance from the creep
pub fn find_nearest_object<R>(creep: &Creep, room: &Room, find_const: impl FindConstant<Item = R>) -> Option<ObjectId<R>>
where
